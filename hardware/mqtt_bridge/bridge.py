...
def _make_mqtt_client(client_id: str):
    """Constructs a paho Client that works on both paho-mqtt 1.x and 2.x.

    paho 2.0 requires an explicit CallbackAPIVersion; we pin VERSION1 so
    the on_connect/on_message callback signatures used in this module stay
    valid on both major versions. On paho 1.x that argument does not
    exist, so we fall back to the legacy constructor.
    """
    clean_session = False
    try:
        from paho.mqtt.client import CallbackAPIVersion
        return mqtt.Client(CallbackAPIVersion.VERSION1, client_id=client_id, clean_session=clean_session)
...
def main() -> None:
    ...
    client = _make_mqtt_client("traceveil-bridge")
