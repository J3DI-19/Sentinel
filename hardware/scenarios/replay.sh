...
# In addition to the receipt-level dedupe, the backend now writes an
# explicit `live.replay_detected` row to audit_events so a defender
# can distinguish an attacker's replay from an operator's accidental
# double-submit (TV5-11).
set -euo pipefail
...
Expected result:
  ...
  * The case's Audit tab shows a NEW row with
    action="live.replay_detected" and details containing source_id,
    sequence, original_receipt_id, and original_received_at (TV5-11).
