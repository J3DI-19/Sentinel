...
| --- | --- | --- |
| `replay.sh`    | Capture one signed frame and republish it. | Bridge forwards; backend returns the ORIGINAL receipt with `duplicate=true`; no new event created (TV5-11). |
| `tamper.sh`    | Modify a metric before republishing. | Rejected as `malformed_live_telemetry`; row in `live_ingest_issues`. |
| `rogue.sh`     | POST with an unregistered token. | HTTP 401; uvicorn access log only; no audit row today (TV5-12). |
