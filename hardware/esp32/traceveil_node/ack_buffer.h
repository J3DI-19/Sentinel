...
public:
  ...
  // Frees the in_use slot with the smallest sent_at_ms (the oldest
  // un-acked frame). Returns its sequence, or 0 if the buffer was empty.
  // Used by the delivery policy as the last resort when the buffer is
  // full AND the synchronous HTTP fallback also failed: the newest frame
  // is preferred and the oldest un-acked one is dropped (a documented
  // loss condition, surfaced to the caller as DROPPED_OLDEST).
  uint32_t evict_oldest() {
    size_t victim = N;
    unsigned long oldest = 0;
    bool found = false;
    for (size_t i = 0; i < N; ++i) {
      if (!slots_[i].in_use) continue;
      if (!found || slots_[i].sent_at_ms < oldest) {
        oldest = slots_[i].sent_at_ms;
        victim = i;
        found = true;
      }
    }
    if (!found) return 0;
    uint32_t seq = slots_[victim].sequence;
    slots_[victim].in_use = false;
    slots_[victim].body   = StringT();
    return seq;
  }

  bool full() const { return occupied() >= N; }
