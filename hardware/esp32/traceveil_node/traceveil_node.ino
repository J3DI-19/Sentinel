...
static bool unacked_track(uint32_t sequence, const String& body) {
  bool ok = g_unacked.track(sequence, body, millis());
  if (!ok) {
    Serial.printf("unacked buffer full; caller must fail seq %u over to HTTP\n",
                  (unsigned)sequence);
  }
  return ok;
}
...
// Publishes over the preferred transport.
//
// Re-review fix: when MQTT succeeds but the ring buffer of unacked
// frames is already full, the previous version logged and returned
// success anyway - the frame could never be re-tried and no ACK would
// arrive to clear its slot. Now a full buffer forces the same
// synchronous HTTPS fallback path a bridge outage would take, so the
// frame reaches the backend on this exact call or the publish is
// reported as failed to the caller.
static bool publish(const String& body, uint32_t sequence) {
  bool ok = false;
#if TV_TRANSPORT_MQTT
  if (mqtt_publish(body)) {
    if (unacked_track(sequence, body)) {
      ok = true;                          // waiting for bridge ack
    } else {
      Serial.println("unacked ring buffer full; forcing HTTP delivery for this frame");
      ok = http_post(body);
    }
