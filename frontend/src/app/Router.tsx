...
function LegacyAssistantRedirect({ path, navigate }: { path: string; navigate: (path: string) => void }) {
  const caseId = path.split("/").filter(Boolean)[1];
  useEffect(() => { navigate(`/assistant?case=${caseId}`); }, [caseId, navigate]);
  return <div className="state-box"><span>◎</span><strong>Opening Investigation Assistant</strong><p>Preserving the selected case context.</p></div>;
}

export function Router() {
  const route=useLocationPath(); let page;
  if(route.path==="/") page=<OverviewCommandCenter navigate={route.navigate}/>;
  else if(route.path==="/assistant") page=<GlobalInvestigationAssistantPage/>;
  else if(route.path==="/cases") page=<CasesPage navigate={route.navigate}/>;
  else if(/^\/cases\/[^/]+\/assistant$/.test(route.path)) page=<LegacyAssistantRedirect path={route.path} navigate={route.navigate}/>;
