  ...
  return <>
    ...
    <Panel className="ai-runtime-panel" title="AI controls">
      ...
      <div className="ai-detail-grid">
        ...
        <div><span>Loaded VRAM</span><b>{formatBytes(ai?.size_vram_bytes)}</b></div><div><span>Capabilities</span><b>{ai?.capabilities?.join(" · ") || "Unavailable"}</b></div><div><span>Grounding packet limit</span><b>{formatBytes(ai?.grounding_context_limit_bytes)}</b></div><div><span>Generation policy</span><b>{ai ? `${ai.max_output_tokens ?? "?"} output · thinking ${ai.thinking_enabled ? "on" : "off"} · temp ${ai.temperature ?? "?"}` : "Unavailable"}</b></div><div><span>Request timeout</span><b>{ai?.generation_timeout_seconds ? `${ai.generation_timeout_seconds} seconds` : "Unavailable"}</b></div>
      </div>
    </Panel>
