...
export function ConnectedCaseWorkspaceV3Page({ path, search = "", navigate }: { path: string; search?: string; navigate: (path: string) => void }) {
  ...
  return <><PageHeader eyebrow={`Persisted investigation · CASE-${id.padStart(4, "0")}`} title={names[section] ?? heading(section)} description={requested === "analytics" ? "Persisted batch investigation analytics are backend-authored from immutable analysis snapshots." : "Connected records preserve backend IDs, UTC timestamps, nulls, provenance, rule traces, and authoritative risk factors."} actions={<><Button onClick={() => navigate(`/cases/${id}/history${snapshotSearch}`)}>Analysis history</Button><Button disabled={loading || refreshing} onClick={refreshWorkspace}>{refreshing ? "Refreshing…" : "Refresh"}</Button><Button onClick={() => navigate(`/import?case=${id}`)}>Import evidence</Button>{requested !== "analytics" && <Button onClick={() => navigate(`/live?case=${id}`)}>Live capture</Button>}<Button variant="primary" disabled={reanalyzing} onClick={reanalyze}>{reanalyzing ? "Reanalyzing…" : "Reanalyze"}</Button></>}/>
