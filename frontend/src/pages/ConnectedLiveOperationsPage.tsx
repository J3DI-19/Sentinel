...
const MAX_EVENTS = 500, MAX_ALERTS = 100, MAX_PENDING = 250, MAX_IDS = 2000;
...
export function ConnectedLiveOperationsPage({ navigate, search = "" }: { navigate: (path: string) => void; search?: string }) {
  ...
  const [events, setEvents] = useState<Record<string, unknown>[]>([]); const [alerts, setAlerts] = useState<Record<string, unknown>[]>([]);
  ...
  const togglePause = () => { if (paused) { const queued = pending.current.splice(0); queued.forEach(apply); if (caseId) void phase3Api.metrics(caseId, session?.session_id).then(setMetrics); } setPaused(value => !value); };
