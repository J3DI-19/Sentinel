...
export function EmptyState({ title, description }: { title: string; description: string }) { return <div className="state-box"><span>◇</span><strong>{title}</strong><p>{description}</p></div>; }
export function LoadingState() { return <div className="state-box"><span className="spinner"/><strong>Loading investigation data</strong></div>; }
export function ErrorState({ message }: { message: string }) { return <div className="state-box state-error"><span>!</span><strong>Unable to load</strong><p>{message}</p></div>; }
