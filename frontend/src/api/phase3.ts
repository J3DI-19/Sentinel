...
export const phase3Api = {
  ...
  createAssistantSession(caseIds: number[] = [], signal?: AbortSignal) { return apiClient.request<{ session_id: string }>("/assistant/sessions", { method: "POST", signal, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ scope: caseIds.length ? "specific_case" : "auto", case_ids: caseIds, reference_ids: [] }) }); },
