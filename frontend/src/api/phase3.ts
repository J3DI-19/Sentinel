...
export const phase3Api = {
  ...
  ask(sessionId: string, question: string, includeEvidence: boolean, signal?: AbortSignal) { return apiClient.request<AssistantJob>(`/assistant/sessions/${sessionId}/messages`, { method: "POST", signal, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ question, include_evidence: includeEvidence }) }); },
