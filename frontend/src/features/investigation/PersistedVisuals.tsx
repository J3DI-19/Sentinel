...
export function PersistedTimeline({ items, onInspect }: { items: Record<string, unknown>[]; onInspect: (item: InvestigationRecordViewModel) => void }) {
  ...
  mapped.forEach(item => {
    ...
    const key = `${minute}|${item.title}|${item.severity ?? "none"}`;
    groups.set(key, [...(groups.get(key) ?? []), item]);
  });
  return <ol className="persisted-timeline" aria-label="Investigation timeline">{Array.from(groups.values()).map(group => {
    const first = group[0]; const last = group[group.length - 1];
    return <li key={`${first.id}-${last.id}`}>
      <div className={`timeline-marker severity-${(first.severity ?? "none").toLowerCase()}`}/>
      <div className="timeline-time"><time>{timelineMinute(first.timestampUtc)}</time><span>{label(first.category ?? first.kind)}</span></div>
      <details className="timeline-card timeline-group"><summary><div><strong>{group.length > 1 ? `${group.length.toLocaleString()} ${first.title} records` : first.title}</strong>{first.severity && <span className={`timeline-severity severity-${first.severity.toLowerCase()}`}>{first.severity}</span>}</div><span>{group.length > 1 ? `${first.timestampUtc ?? "Time unavailable"} — ${last.timestampUtc ?? "Time unavailable"}` : first.id}</span><b>{group.length > 1 ? "Show raw records" : "Show record"}</b></summary><div className="timeline-group-records">{group.map(item => <div key={item.id}><code>{item.id}</code><time>{item.timestampUtc ?? "Time unavailable"}</time><button className="record-inspect" onClick={() => onInspect(item)} aria-label={`Inspect ${item.id}`}>Inspect details</button></div>)}</div></details>
    </li>;
  })}</ol>;
}
...
export function TimelineActivityOverview({ points, total }: { points: ChartPoint[]; total: number }) {
  ...
  return <section className="timeline-overview"><header><div><span>Case activity</span><h2>Timeline summary</h2><p>{total.toLocaleString()} persisted entries. The chart covers the complete snapshot; grouped records below cover the displayed page.</p></div></header>{activity.length ? <div className="timeline-overview-chart"><ResponsiveContainer width="100%" height="100%"><AreaChart data={activity}><defs><linearGradient id="timelineActivity" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#27c2e8" stopOpacity={0.4}/><stop offset="1" stopColor="#27c2e8" stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke="#1b2a40" vertical={false}/><XAxis dataKey="time" stroke="#71839a" minTickGap={30}/><YAxis stroke="#71839a" allowDecimals={false}/><Tooltip contentStyle={tooltipStyle}/><Area type="monotone" dataKey="value" name="Events" stroke="#27c2e8" strokeWidth={2} fill="url(#timelineActivity)"/></AreaChart></ResponsiveContainer></div> : <VisualEmpty text="No complete-snapshot activity aggregate is available."/>}</section>;
}
