...
export function ValidationSummary({ result, capability = "mock" }: { result: ImportValidationResult | null; capability?: "mock" | "backend" }) {
  ...
  const groups = result ? (["file", "column", "row"] as const).map(scope => ({ scope, problems: result.problems.filter(problem => problem.scope === scope) })).filter(group => group.problems.length) : [];
  return <Panel title="Validation summary">{!result ? <div className="import-empty"><span aria-hidden="true">◇</span><b>No validation result</b><p>Results appear only after a selected file is processed by the {capability === "mock" ? "mock adapter" : "connected API"}.</p></div> : <><div className="validation-score"><div><b>{result.acceptedRecords}</b><span>Accepted</span></div><p>{result.detectedSchema}<br/>Detected source: {result.detectedSource.replaceAll("_", " ")}</p></div><dl className="validation-list"><div><dt>Source records</dt><dd>{result.totalRecords}</dd></div><div><dt>Accepted</dt><dd className="success">{result.acceptedRecords}</dd></div><div><dt>Rejected</dt><dd className={result.rejectedRecords ? "danger" : ""}>{result.rejectedRecords}</dd></div><div><dt>Duplicates</dt><dd>{result.duplicateRecords}</dd></div></dl>{result.warnings.map(warning => <p className="import-warning" key={warning}>{warning}</p>)}{groups.map(group => <section className="rejection-group" key={group.scope}><h4>{group.scope} problems</h4><div className="rejections" aria-label={`${group.scope} validation problems`}>{group.problems.map((problem, index) => <div key={`${problem.code}-${problem.row ?? "file"}-${problem.column ?? "none"}-${index}`}><i>!</i><div><b>{problem.code}</b><span className="problem-location">{[problem.row ? `Row ${problem.row}` : null, problem.column ? `Field ${problem.column}` : null].filter(Boolean).join(" · ") || "Whole file"}</span><span>{problem.message}</span>{problem.correction && <span className="problem-correction">Fix: {problem.correction}</span>}</div></div>)}</div></section>)}{result.rejectionLogReference && <p className="import-warning"><b>Rejection reference:</b> <code>{result.rejectionLogReference}</code> <Button onClick={copyReference}>Copy reference</Button></p>}</>}</Panel>;
}

export function ImportActions({ state, hasFile, hasValidation, retryable, onValidate, onImport, onCancel, onRetry, onOpenResults, capability = "mock", partial = false, acceptedRecords = 0, rejectedRecords = 0, partialApproved = false, onPartialApproved = () => {} }: { state: ImportWorkflowState; hasFile: boolean; hasValidation: boolean; retryable: boolean; onValidate: () => void; onImport: () => void; onCancel: () => void; onRetry: () => void; onOpenResults?: () => void; capability?: "mock" | "backend"; partial?: boolean; acceptedRecords?: number; rejectedRecords?: number; partialApproved?: boolean; onPartialApproved?: (approved: boolean) => void }) {
  ...
  const completed = state === "success" || state === "partial-success";
  return <div className="import-result"><div><span>{mock ? "Mock adapter" : "Connected API"}</span><b>{mock ? "Frontend-only workflow" : "Persisted batch workflow"}</b><p>{mock ? "Processing is deterministic demonstration behavior. Nothing is uploaded, persisted, or forensically analyzed." : partial ? "Approval persists accepted records; rejected rows remain excluded and are not canonical evidence." : "Validation is non-destructive. Commit persists accepted records and deterministic analysis."}</p></div>{state === "reviewing" && partial && <label className="partial-approval"><input type="checkbox" checked={partialApproved} onChange={event => onPartialApproved(event.target.checked)}/><span><b>Import {acceptedRecords} accepted records and exclude {rejectedRecords} rejected records.</b> Rejected rows will not become canonical evidence.</span></label>}{completed && !mock && onOpenResults ? <Button variant="primary" onClick={onOpenResults}>Open case results →</Button> : state === "reviewing" ? <Button variant="primary" onClick={onImport} disabled={!hasValidation || (partial && !partialApproved)}>{partial ? "Approve partial import" : `Import with ${adapter}`} →</Button> : (state === "failed" || state === "cancelled") && retryable ? <Button variant="primary" onClick={onRetry} disabled={!hasFile}>Retry processing</Button> : <Button variant="primary" onClick={onValidate} disabled={!hasFile || busy || completed}>Validate with {adapter} →</Button>}{busy && <Button variant="danger" onClick={onCancel}>Cancel processing</Button>}<small>{mock ? "Mock adapter · No backend connection or persistence" : "Connected API · Original evidence is content-addressed and immutable"}</small></div>;
}
