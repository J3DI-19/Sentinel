...
Steps 1 through 3 and the core Step 4 and Step 6 services are implemented. Step 4 remains incomplete for the selected evaluation scope until dedicated CASAS and TON_IoT device-telemetry profiles and adapters are added. The repository contains the React/Vite application shell, FastAPI health endpoints, SQLite initialization, frontend/backend connectivity, dependency manifests, tests, local Ollama/Qwen setup, backend evidence validation, the versioned canonical event/normalization pipeline, and the deterministic investigation engine. The frontend includes navigation, case workspaces, evidence import, investigation dashboards, deterministic visualization placeholders, timelines, entity graphs, findings, live-monitoring views, the Investigation Assistant, and report review flows.

Step 2 interfaces currently operate on structured mock data. Step 3 supplies deterministic validation and the shared live-telemetry input contract. Step 4 provides the strict canonical schema plus simulated, generic, accepted-live, TON_IoT-network, and CICIoT2023-network compatibility adapters; dedicated CASAS and TON_IoT device-telemetry adapters remain to be implemented for the selected evaluation scope. Step 6 applies versioned filters, stable sorting, rules, behavioural baselines, bounded factorized scoring, correlation, incidents, timelines, graphs, aggregates, and live-trigger alert creation. Live collection, real-time delivery, investigation APIs, production AI integration, automation, and the physical demonstration remain planned work. See the [validation contract](docs/contracts/evidence-validation.md), [canonical event contract](docs/contracts/canonical-event.md), [deterministic analysis contract](docs/contracts/deterministic-analysis.md), and [roadmap](docs/roadmap.md).

Implementation can now proceed with the CASAS and TON_IoT telemetry profiles/adapters, Step 5 live collection, Step 7 real-time delivery, and Step 8 investigation APIs against the versioned validation, canonical-event, and deterministic-analysis contracts.
