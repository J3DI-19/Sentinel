...
class TonIotFridgeTelemetryAdapter(CanonicalAdapter):
...
class LiveTelemetryAdapter(CanonicalAdapter):
    ...
    def normalize(self, record: dict[str, Any], context: NormalizationContext) -> CanonicalEvent:
        ...
        actor = (
            CanonicalEntity(id=context.source_id, kind=EntityKind.SOURCE)
            if source_event_type == "authentication" and context.source_id
            else None
        )
        return self._build_event(
            record=record,
