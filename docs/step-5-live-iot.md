...
Rows are rate-limited per (source_id or client_ip) to
`LIVE_AUTH_FAILURE_AUDIT_PER_MINUTE` (default 6) rows/minute so a
brute-force flood cannot fill audit_events. The auth-failure row's
`case_id` is NULL because the source is not attached to any active
session; query it directly on `audit_events`.

A replay of a previously accepted `(source_id, sequence)` still
returns the ORIGINAL receipt with `duplicate=true` unchanged, and the
backend now writes an additional `live.replay_detected` audit row
(TV5-11) carrying `source_id`, `sequence`, `original_receipt_id`,
and `original_received_at` so a defender scanning the case audit
can distinguish an attacker's replay from an operator's accidental
double-submit.
