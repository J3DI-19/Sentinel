...
pytest tests/test_live_telemetry_contract.py \
       ...
       tests/test_mqtt_bridge_forwarding.py -q
```
...
| G2 | Working mutual TLS | `configure_tls` refuses to start without CA/cert/key; sketch `assert_transport_config()` halts if PEMs are absent; CI job `bridge-broker` runs the real handshake. |
| G3 | Per-device topic authorization | `hardware/mqtt_bridge/traceveil.acl` pins each CN to its own topics; CI `assert_topic_acl_enforced.sh` proves node A cannot publish as node B. |
| G4 | Valid time source | Sketch `await_ntp_or_halt()` blocks publication until NTP passes `TV_NTP_MIN_EPOCH`. |
| G5 | Durable delivery | Bridge SQLite spool + backoff + dead-letter; `test_spool_survives_backend_outage_then_recovers` covers the outage-recovery path. |
| G6 | Accurate scenario behavior | Scenario scripts and `docs/step-5-live-iot.md` describe what the backend actually does (replay = duplicate receipt; rogue = HTTP 401 only). |
| G7 | Protected transport | `TV_HTTP_TLS` defaults to 1; sketch halts if plaintext is used against a non-loopback host. |
| G8 | Expanded CI | `.github/workflows/step5-integration.yml` runs unit + broker + arduino-cli-compile jobs. |
